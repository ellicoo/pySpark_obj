...
df = spark.read.parquet(
    "s3://ngap--customer-data-science--prod--us-east-1/airbot/prod/dening/other_human_all_community_id_data_handled_Enhance_v1/")
print(f"数据源数据量：{df.count()}")
community_132_df = df.filter(F.col("community_id") == 132)
normal_address_df = community_132_df.filter(
    (F.col("address_1_2_like_count") == 1) & (F.col("address_1_3_like_count") <= 10) & (
                F.col("address_2_3_like_count") <= 10) & (F.col("address_2_4_like_count") <= 10) & (
                F.col("address_3_4_like_count") <= 10) & (F.col("address_3_5_like_count") <= 10))

list1 = ["9aed82fa-b3b1-40b2-9426-5ed7d4c0bbad", "0e6b397a-deac-47d2-91ca-ed487007d2dd",
         "caa0840a-b17c-4947-9013-79b6a43a4719", "65c891c2-21ec-43ac-8620-2dedd4c85c0c",
         "9aed82fa-b3b1-40b2-9426-5ed7d4c0bbad"]
...
new_df = normal_address_df.join(mid_df, normal_address_df.upmid == mid_df.upmid, "inner").drop(mid_df.upmid).select(
    "upmid", "address", "community_id")
new_df.display()
