...
#     l_after = transform_list(l_before)
#     return l_after
...
#中间测试
text = "3-26-12-502 パークホームズ板橋蓮根二番街 東京都"
segmented_result, room_nums = segmentation(text)
print("分词结果:", segmented_result)
print("被替换的数字:", room_nums)
...
# 函数的功能测试
data=["5-7-11","3E","ff-2f","B2","aa56", "s12","768999s","a233","sorvanilladu959"]

convert_list_to_str_for_bert(data)


# 1.Tag RANDOM｜ROOM｜CORRECT jp’s address
# 全量
def get_all_df():
    ...
    # 将元组拆分为两列
    df = df.withColumn("segmented_result", segmentation_udf("address")) \
    ...
    # 去重
    final_data = df.dropDuplicates(["account_id", "address"])

    # final_data.createOrReplaceTempView('jp_tag')
    print(f"打完标签后的数据量：{final_data.count()}")
    return final_data
...
# Tag ZIPCODE_ERROR to jp’s addre
...
def get_type1_zipcode_shipping_address_processed_address_df():
    from pyspark.sql import functions as F
    import re
    ...
    # 获取zipcode的table
    # 此处(F.col("account_id").isNotNull())就是避免数据倾斜的做法
...
print("------------------------add three tag to full_amount------------------------------")

# 基于get_all_df()打的tag标签，增加一个zipcode_error标签
# 此处做个修正，尝试解决AnalysisException: Cannot overwrite a path that is also being read from.
# get_history_new_processed_df()

new_jp_tag_df = get_all_df().withColumnRenamed("account_id", "upmid").select(
    "upmid",
    "address",
    "parsed_address",
    "mask_messycode",
...
# spark.sql("""
# insert overwrite table airbot_prod.jp_community_tag_dev
# select a.*,b.community_id
# from jp_tag_v2 as a
#  inner join airbot_prod.jp_community b on a.account_id=b.forter_address__account_id_list_for_the_address and a.address=b.address
# """)

print("------------------------add zipcode_tag------------------------------")
print(f"join type1、中间表(包含account_id&shipping_address&address)、jp_community_tag_dev 后的数量：{result_df.count()}")
print(f'ZIPCODE_ERROR数量：{result_df.filter(F.col("zipcode_digits") != 7).count()}')
# print(f'全量ZIPCODE_ERROR数量：{last_df.filter(F.col("zipcode_digits")!=7).count()}')
print(f"最终打到标签的数据量：{result_df.count()}")
# 将临时视图保存为临时的delta表
result_df.createOrReplaceTempView("jp_tag_v2")
...
# # 插入原表之前做的中间表
# spark.sql("""
# create table airbot_prod.jp_community_tag_dev_temp
# using parquet
# location 's3://ngap--customer-data-science--prod--us-east-1/airbot/prod/dening/jp_community_tag_backup_tmp'
# as
#   select a.*,b.community_id from jp_tag_v2 as a
#   inner join airbot_prod.jp_community b on a.account_id=b.forter_address__account_id_list_for_the_address and a.address=b.address
# """)
...
# # 创建终表
# spark.sql("""
# create table airbot_prod.jp_community_tag_dev
# using parquet
# location 's3://ngap--customer-data-science--prod--us-east-1/airbot/prod/dening/jp_community_tag_backup'
# as
#   select * from airbot_prod.jp_community_tag_dev_temp
# """)
