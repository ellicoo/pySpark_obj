# 本代码使用的是databricks环境进行的编辑
# databricks的notebook本身已经初始化了一个spark对象
# 一些包的依赖
...
# 对中间表与type1进行链接操作打完成zipcode标签的前置准备
...
print(f"增量join type1、中间表(包含account_id&shipping_address&address)、jp_community_tag_dev 后的数量：{result_df.count()}")
...
print(f"(增量)与历史数据合后并的数据量：{last_df.count()}")
